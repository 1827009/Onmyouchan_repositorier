...
namespace onmyouchan.Entity
{
    ...
    class Enemy : Character
    {
        ...
        public void StopItem(float time)
        {
            controlWeit = time;
        }
            ...
            if ((vec.X * vec.X) + (vec.Y * vec.Y) + (vec.Z * vec.Z) <= r * r)
            {
                ...
                if (entity.thisEntity is Player)
                {
                    try
                    {
                        ...
                        if (thisEntity.ThisEntityManager.player.knockbackTime <= 0&&thisEntity.ThisEntityManager.player.invincibleTime<=0)
                        {
                            thisEntity.ThisEntityManager.player.damage += enemy.Attack;
                            thisEntity.ThisEntityManager.player.knockbackTime = 0.7f;
                            entity.thisEntity.MoveStatus.moveVec += reboundDirection * (resilience * 20);
                            thisEntity.ThisEntityManager.player.goalAngle = (float)Math.Atan2(vec.Z, -vec.X);
                            thisEntity.ThisEntityManager.player.invincibleTime = 1;
                        }
                    }
                    catch
                    {
